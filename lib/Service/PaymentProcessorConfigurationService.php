...
class PaymentProcessorConfigurationService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			...
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Secupay\Sdk\Model\PaymentProcessorConfiguration', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Secupay\Sdk\Model\EntityQuery $query The query restricts the payment processor configuration which are returned by the search. (required)
	 * @throws \Secupay\Sdk\ApiException
	 * @throws \Secupay\Sdk\VersioningException
	 * @throws \Secupay\Sdk\Http\ConnectionException
	 * @return \Secupay\Sdk\Model\PaymentProcessorConfiguration[]
