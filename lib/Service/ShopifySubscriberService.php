...
class ShopifySubscriberService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Secupay\Sdk\Model\ShopifySubscriber',
				'/shopify-subscriber/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Secupay\Sdk\Model\ShopifySubscriber', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Secupay\Sdk\Model\EntityQuery $query The query restricts the Shopify subscribers which are returned by the search. (required)
	 * @throws \Secupay\Sdk\ApiException
	 * @throws \Secupay\Sdk\VersioningException
	 * @throws \Secupay\Sdk\Http\ConnectionException
	 * @return \Secupay\Sdk\Model\ShopifySubscriber[]
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Secupay\Sdk\Model\ShopifySubscriberActive $query The Shopify subscriber object with all the properties which should be updated. The id and the version are required properties. (required)
	 * @throws \Secupay\Sdk\ApiException
	 * @throws \Secupay\Sdk\VersioningException
	 * @throws \Secupay\Sdk\Http\ConnectionException
	 * @return \Secupay\Sdk\Model\ShopifySubscriber
