...
class WebAppService {
	 ...
	 * Confirm
	 *
	 * @param \Secupay\Sdk\Model\WebAppConfirmationRequest $request  (required)
	 * @throws \Secupay\Sdk\ApiException
	 * @throws \Secupay\Sdk\VersioningException
	 * @throws \Secupay\Sdk\Http\ConnectionException
	 * @return \Secupay\Sdk\Model\WebAppConfirmationResponse
	 */
	...
	public function confirmWithHttpInfo($request) {
		...
		try {
			...
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Secupay\Sdk\Model\WebAppConfirmationResponse', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
